...
fn main() -> Result<()>{
    ...
    let mut to_adapter: VecDeque<Box<ndisapi::driver::driver::IntermediateBuffer>> = VecDeque::new();
    for _i in 0..256 {
        ib.push_back(Box::new(ndisapi::driver::driver::IntermediateBuffer::default()));
    }
    ...
    while {
        packets_read = driver.read_packets::<_, 256>(adapters[interface_idx].handle, ib.iter_mut());
         packets_read > 0 }{
        ...
        if to_adapter.is_empty() == false{
            driver.send_packets_to_adapter::<_, 256>(adapters[interface_idx].handle, to_adapter.iter_mut());
            ib.append(&mut to_adapter);
        }

        if to_mstcp.is_empty() == false {
            driver.send_packets_to_mstcp::<_, 256>(adapters[interface_idx].handle, to_mstcp.iter_mut());
